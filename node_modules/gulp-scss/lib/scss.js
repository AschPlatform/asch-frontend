(function() {
  var applySourceMap, compile, dargs, gutil, inject, mapConverter, merge, path, q, testing, through;
    ...
    "exec", "mkdirp", "fs", function(exec, mkdirp, fs) {
      return function(opts) {
        return function(file, enc, cb) {
          var compilerPromise, options, passedArgs, tmpDir;
          if (file.isNull()) {
            return cb(null, file);
          }
          options = {
          ...
          q.nfcall(mkdirp, tmpDir).then(function() {
            ...
            command.push("scss");
            command = command.concat(passedArgs, file.path, path.join(tmpDir, gutil.replaceExtension(path.basename(file.relative), ".css")));
